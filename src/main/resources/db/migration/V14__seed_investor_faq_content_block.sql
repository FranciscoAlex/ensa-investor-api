-- Seed investor FAQ content block for the new /api/v1/investor-content/investor-faq endpoint.
-- Keep existing edited content untouched by inserting only when key does not exist.
...
WHERE NOT EXISTS (
    SELECT 1
    FROM content_blocks
    WHERE block_key = 'investor_faq.json'
);
